...
export function TrialPromoDialog(
  ...
  // Ao ir para o checkout e voltar pelo navegador (botão/tecla de retorno), a página
  // é restaurada do bfcache com o loading ("aplicando") ainda ativo — limpa o estado
  // nesse retorno (pageshow) para não ficar preso na tela de carregamento.
  useEffect(() => {
  ...
  const supportBtn = <WhatsAppSupport url={user?.support_whatsapp_url} label={t("plans:supportWhatsapp")} />;
  ...
  async function choose(planId: number) {
    ...
    // Vencido/SEM PLANO: clicar no "plano atual" NÃO fecha (senão burlaria o bloqueio) —
    // segue para a renovação. Só dispensa no modo promo (não vencido).
    if (!expired && planId === cur?.plan_id) { onClose(); return; }
